...
public static void main(String[] args) {
	...
	List<Fruit> sortedList=al.stream().filter((e)->e.getCalories()<100).sorted(Comparator.comparing(Fruit::getCalories)).collect(Collectors.toList());
	System.out.println(sortedList);
	List<Fruit> listByColor=al.stream().sorted(Comparator.comparing(Fruit::getColor)).collect(Collectors.toList());
    ...
    List<Transaction> sortedListByValue=transList.stream().filter((e)->e.getYear()==2011).sorted(Comparator.comparing(Transaction::getValue)).collect(Collectors.toList());
    System.out.println(sortedListByValue);
    List<String> cities=transList.stream().map((e)->e.getTrader().getCity()).distinct().collect(Collectors.toList());
    System.out.println(cities);
    List<Transaction> sortingByName=transList.stream().filter((e)->e.getTrader().getCity()=="pune").collect(Collectors.toList());
    System.out.println(sortingByName);
    //List<Trader> sortingByNameInAlph=transList.stream().sorted(Comparator.comparing(Trader::getName)).collect(Collectors.toList());
    List<Transaction> delhiList= transList.stream().filter((e)->e.getTrader().getCity()=="Delhi").collect(Collectors.toList());
